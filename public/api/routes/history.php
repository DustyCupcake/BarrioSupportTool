...
function handle_history(): void {
    ...
    $type   = $_GET['type'] ?? null;
    $bid    = (int)($_GET['group_id'] ?? $_GET['barrio_id'] ?? 0);
    ...
    if ($include_equip) {
        ...
        if ($bid > 0) {
            $equip_where[]  = "t.holder_type = 'group' AND t.holder_id = ?";
            $equip_params[] = $bid;
        }
        ...
        $parts[]      = "SELECT t.id, t.type, t.is_offline_entry, t.occurred_at, t.notes,
         ...
         LEFT JOIN groups g      ON g.id = t.holder_id AND t.holder_type = 'group'
         $ew";
